...
export default function UserProfileCard({ user }) {
  // If user is null or undefined, show a placeholder card
  if (!user) {
    return (
      <div className="bg-indigo-50 dark:bg-gray-700 rounded-lg p-4 flex flex-col sm:flex-row items-center gap-4">
        <div className="flex-shrink-0">
          <div className="h-20 w-20 rounded-full bg-white dark:bg-gray-600 border-2 border-indigo-200 dark:border-indigo-800 flex items-center justify-center text-2xl text-gray-400 dark:text-gray-500">
            ?
          </div>
        </div>
  ...
  // Format joined date using our utility function
  const joinedDate = user.joinedDate 
    ? formatDate(user.joinedDate, 'MMMM yyyy') 
    : user.createdAt
      ...
      <div className="text-center sm:text-left">
        ...
        <div className="mt-2 flex items-center justify-center sm:justify-start">
          ...
          <span className="text-gray-600 dark:text-gray-300">
            {user.completedServices || 0} services completed
          </span>
        </div>
      </div>
